...
void graphdefaults( )
{
    ...
    */
    ...
    BGI__ReleaseWinbgiDC( );//Fix by Tony The Magnificent
}
...
( int width, int height, const char* title, int left, int top, bool dbflag , bool closeflag)
{
    ...
    pWndData->FullScreen = false;
    pWndData->HiRes = false;
...
// This function will change window to fullscreen mode and back to windowed mode on each call,
// and return the size of window, but his doesn't works correctly, and I don't know why.
//
/*
void changewindowmode(int& width, int& height)
{
    WindowData *pWndData = BGI__GetWindowDataPtr( );

    static int WindowedWidth = pWndData->width, WindowedHeight = pWndData->height;

    if ( !pWndData->FullScreen )
    {
        pWndData->width = width = GetSystemMetrics(SM_CXSCREEN);
        pWndData->height = height = GetSystemMetrics(SM_CYSCREEN);
        SetWindowLong(pWndData->hWnd, GWL_STYLE, WS_POPUP);
        SetWindowPos(pWndData->hWnd, HWND_TOP, 0, 0, width, height, SWP_HIDEWINDOW | SWP_DEFERERASE | SWP_FRAMECHANGED);
        pWndData->FullScreen = true;
    }
    else
    {
        int CaptionHeight;
        ...
        pWndData->width = width = WindowedWidth + CaptionHeight + 2*yBorder;
        pWndData->height = height = WindowedHeight + 2*xBorder;
        SetWindowLong(pWndData->hWnd, GWL_STYLE, pWndData->WindowStyles);
        SetWindowPos(pWndData->hWnd, NULL, pWndData->initleft, pWndData->inittop, width, height, SWP_HIDEWINDOW | SWP_DEFERERASE | SWP_FRAMECHANGED | SWP_NOZORDER);
        pWndData->FullScreen = false;
    }

    HDC hDC = GetDC( pWndData->hWnd );
    HBITMAP hBitmap;
    pWndData->hDCMutex = CreateMutex(NULL, FALSE, NULL);
    WaitForSingleObject(pWndData->hDCMutex, 5000);
    for ( int i = 0; i < MAX_PAGES; i++ )
    {
        DeletePen( SelectPen( pWndData->hDC[i], GetStockPen( BLACK_PEN ) ) );
        DeleteBrush( SelectBrush( pWndData->hDC[i], GetStockBrush( BLACK_BRUSH ) ) );
        ...
        SetTextColor( pWndData->hDC[i], RGB(0,0,0) );
    }
    ReleaseMutex(pWndData->hDCMutex);

    ReleaseDC( pWndData->hWnd, hDC );
    ...
    ShowWindow( pWndData->hWnd, pWndData->FullScreen ? SW_SHOWMAXIMIZED : SW_SHOWNORMAL );
    UpdateWindow( pWndData->hWnd );
}*/
